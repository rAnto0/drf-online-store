...
class OrderModelTest(TestCase):
    def setUp(self):
        ...
        self.order = Order.objects.create(user=self.user, total_amount=Decimal("0.00"))
    ...
    def test_order_total_amount(self):
        ...
        expected_total = (
            self.order_item1.quantity * self.order_item1.price
...
class OrderItemModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="orderuser2", email="order2@example.com", password="password123"
        )
        self.category = Category.objects.create(
